...
function main()
{
    ...
    if(config.lang==null)
    {
        $.getJSON("//rxat.ro/json/lang/locate.php?v="+config.xcb_l, function(o)
        {
    ...
    //load languages
    $.getJSON("//rxat.ro/json/lang/languages.php?v="+config.xcb_l, function(o)
    {
        var s = '';
        for(var i in o)
            if(o[i].i)
        ...
        s += '<li><a href="//rxat.ro/web_gear/chat/changelanguage.php">More...</a></li>';
        $("#langselect").html(s);
        ...
        $('[data-lang]').click(function(e)
        {
            if(!config.cookiedecline)
			{
			    ...
			    $.cookie('clang', $(this).attr("data-lang"), { domain: config.cookieDomain, path:'/' });
			}
            window.location = '//rxat.ro';
            return false;
        });
    });
    ...
    if($("meta[name=xcookie]").attr('content')!='no')
    {
        $.cookieCuttr(
        {
            ...
            cookieErrorMessage: "Features that need <a href=\"//rxat.ro/privacy.html\">cookies</a> have been disabled as you have DECLINED cookies.<br><br>To continue using xat please:",
            cookieDisable: ".warning",
            ...
            cookiePolicyLink: '//rxat.ro/privacy.html',
	...
	if(js!='none')
	    $.getScript("//rxat.ro/js/"+js+".js?"+config.xcb_js, function() { if(typeof pmain === 'function') pmain(); });
	else
...
function localize(c)
{
    if(config.lang!="en")
    {
        ...
        var NewLang = ($.cookie('zz'));

        //load translations
        for(key in lf)
        {
            var path = "//rxat.ro/json/translate/translate.php?";
            var ext = "&v="+ config.xcb_l;
            // for all if(NewLang) 
                ext += "&new=1";
	...
	else if(config.lang === 'en' && urlParam("upd"))
    {
        ...
        $.post("//rxat.ro/json/lang/english.php", {uid:config.doTranslate, json:JSON.stringify(jse)},
...
function localizeDone()
{
    ...
    if(config.doTranslate)
    {
        ...
        $("#savetranslation").click(function() {
            ...
            $.post("//rxat.ro/json/lang/translate.php", {json:JSON.stringify(jt)}, 
				function(result){if(result!=='0') alert(result);});
        });

        //color code the [t] links
        path = "//rxat.ro/json/lang/coloring.php?uid="+ config.doTranslate+"&lang="+config.lang+"&file="+$("meta[name=xt]").attr('content');  
...
var adx = [
["doodleracead", "//web.rxat.ro/games_chat.php"],
["snakeracead", "//web.rxat.ro/games_chat.php"],
["matchracead", "//web.rxat.ro/games_chat.php"],
["spacewarad", "//web.rxat.ro/games_chat.php"]
];

adx = [
["xatAd", "//rxat.ro/powers"]
];

function ads()
{
	adindex=0;
    $.getJSON("//rxat.ro/json/adverts.php?c="+config.cb60, function(o) { 
...
function adnew()
{
	...
	{	
        ...
        {
            var u = adverts[j]["adimg"];
            if(adverts[j]["base64"]) u = "chatsgroup.com/css/adpic.php?u="+u;
			s='<a href="//rxat.ro/'+adverts[j]["group"]+'"><img border="0" src="//'+u+'" width="728" height="90" /></a>';
		}
        else
		{
			j = adxi % adx.length;
			s='<div onmousedown="GoAd('+j+');"><embed wmode="transparent" src="//rxat.ro/images/ads/'+adx[j][0]+'.swf?b9" quality="high" flashvars="" width="728" height="90" align="middle" allowscriptaccess="sameDomain" type="application/x-shockwave-flash" /></div>';
			adxi++;
		}
		$(this).html(s);
		adindex++;
